...
Runs the trained model on all shifu-student pairs in the dataset and
outputs a formatted results table. For each pair, loads both CSVs,
fills SlidingWindowBuffers to capacity, runs a single inference pass,
and reports per-limb scores + overall score alongside the ground-truth.

Matched pairs: first shifu of each movement vs all students of the
same movement. Expected behaviour:
    - score=1.0 students should produce high model scores (~0.8+)
    - score=0.1 students should produce low model scores (~0.0-0.2)

Mismatched pairs (--mismatches flag): each shifu vs the best student
(score=1.0) of every OTHER movement. These should all produce near-zero
overall scores, validating the open-set boundary. If mismatch scores
are high, the model is not discriminating between movement types.
