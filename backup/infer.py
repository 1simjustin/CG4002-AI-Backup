...
through a SlidingWindowBuffer, triggering inference every --step_size frames.
...
def load_model(checkpoint_path, device):
    """Load model and normalisation stats from checkpoint."""
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)

    model = AQAModel()
    # strict=False allows loading old checkpoints that lack log_temperature
