...
During training, ~15% of pairs are synthetic mismatches (different movement)
with a target score of 0.0, enabling open-set comparison at inference time.

Input normalisation (two stages):
    1. Per-sample mean subtraction: removes session-specific gravity/orientation
       offset so the model sees motion deltas, not absolute sensor orientation.
    2. Global z-score: zero-mean, unit-variance per channel computed from all
       training CSVs, so accel and gyro channels are on the same scale.
"""
...
def _augment(tensor: torch.Tensor, jitter_std: float, scale_range: Tuple[float, float]) -> torch.Tensor:
    """Apply jitter and scaling independently, each with 50% probability."""
...
class AQADataset(Dataset):
    """
    Pairs student IMU recordings with shifu recordings for quality scoring.

    Args:
        recordings_dir: Folder with CSVs and labels.json.
        max_seq_len: Fixed sequence length (truncate/pad).
        augment: Enable jitter + scaling (training only).
        jitter_std: Gaussian noise std for augmentation.
        scale_range: (low, high) uniform multiplier for augmentation.
        negative_pair_prob: Chance of replacing shifu with a different movement.
    """
...
if __name__ == "__main__":
    ...
    ds = AQADataset(RECORDINGS, max_seq_len=128, augment=True, negative_pair_prob=0.15)
    ...
    loader = get_aqa_dataloader(RECORDINGS, batch_size=4, max_seq_len=128,
                                augment=True, negative_pair_prob=0.15)
